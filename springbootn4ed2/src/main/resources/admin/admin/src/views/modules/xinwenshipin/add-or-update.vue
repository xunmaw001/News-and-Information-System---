<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="视频标题" prop="shipinbiaoti">
          <el-input v-model="ruleForm.shipinbiaoti" 
              placeholder="视频标题" clearable  :readonly="ro.shipinbiaoti"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="视频标题" prop="shipinbiaoti">
              <el-input v-model="ruleForm.shipinbiaoti" 
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `xinwenshipin/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xinwenshipinCrossAddOrUpdateFlag = false;
                  this.parent.search();
