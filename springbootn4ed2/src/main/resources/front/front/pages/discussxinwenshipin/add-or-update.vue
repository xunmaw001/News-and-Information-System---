	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussxinwenshipin`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(this.ruleForm.id){
					await this.$api.update(`discussxinwenshipin`, this.ruleForm);
				}else{
					await this.$api.add(`discussxinwenshipin`, this.ruleForm);
				}
				uni.setStorageSync('discussxinwenshipinCleanType',true);
