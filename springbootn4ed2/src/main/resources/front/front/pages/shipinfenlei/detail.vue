...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','shipinfenlei');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','shipinfenlei');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('shipinfenlei', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussshipinfenlei/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('shipinfenlei', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('shipinfenlei', this.detail);
				this.$utils.msg('审核成功');
